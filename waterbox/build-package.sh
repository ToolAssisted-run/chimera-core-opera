#!/bin/sh
# Builds the Opera (3DO) waterbox core package and installs it into a chimera
# checkout as build/Cores/opera.zip.
#
...
[ -n "$mb" ] || mb="$chimera_root/extern/miniBox"
...
opera_pin="$(git -C "$root/extern/opera-libretro" describe --tags --always 2>/dev/null || echo unknown)"
...
json.dump({
    ...
    "upstream": {"name": "Opera (opera-libretro)", "pin": "$opera_pin"},
    "builtOn": "$os_id",
...
for cache in "$chimera_root"/build/CoreCache/opera-*; do
