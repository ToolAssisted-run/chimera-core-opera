...
if len(sys.argv) > 4:
    ...
    for decl_id, path in json.loads(sys.argv[4]).items():
        fw["Opera/" + decl_id] = path
# Pick this core explicitly: an install can hold several packages claiming the 3DO system, and --core only LOADS a package, it does not choose it.
cfg.setdefault("DefaultCores", {})["3DO"] = "Opera"
